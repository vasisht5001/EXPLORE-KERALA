...
const Login = async (req, res) => {
    try {
        ...
        return res.status(200).json({ success: true, message: "Login successfull", user: FindUser, token });
    } catch (error) {
    ...
    try {
        ...
        if (req.file) {
            ...
            if (ExistUser.profile) {
                const oldImagePath = `public/images/${ExistUser.profile}`;
                if (fs.existsSync(oldImagePath)) {
                    fs.unlinkSync(oldImagePath);
                }
            }
            ExistUser.profile = req.file.filename; // Save new filename
        }
        ...
        res.status(200).json({
            success: true,
            message: "Profile updated successfully. please signout and sign in again to access admin features",
            user: {
                _id: ExistUser._id,
